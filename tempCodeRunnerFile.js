...
// Approve/Reject Publisher (with book file deletion if rejected)
app.put("/api/admin/publishers/:id/approve", requireAdminAuth, async (req, res) => {
  try {
    ...
    if (approve) {
      ...
      // If rejecting publisher, delete all their books and files first
      const publisherBooks = await Book.find({ uploadedBy: publisherId });
      ...
      await Book.deleteMany({ uploadedBy: publisherId });
      ...
      console.log('❌ Publisher rejected and all data deleted:', publisher.email);
      
      res.json({
        success: true,
        message: 'Publisher rejected and all associated data removed'
      });
    }
...
app.put("/api/admin/books/:id/status", requireAdminAuth, async (req, res) => {
  try {
    ...
    await book.populate('uploadedBy', 'fullName email');
